...
> 快照日期：2026-06-12
> 本地工作目录：`E:\Codex_Projects\HyperSCA`
> GitHub 对照：本轮实现前本地 `main` 与 `luvega/HyperSCA` 远端 `main` 均为 `3ee78ba348f6584eb892d86264d40311177cd0c4`；当前开发分支为 `codex/behavior-grammar-stage5`。

本文用于界定当前项目中的四类内容：项目提交代码、验证代码、本地数据、运行结果。`data/`、`results/`、`references/`、`.venv/`（如存在）、`.Trash/` 均为本地资产，默认不进入 Git。

## 1. 仓库边界

| 路径 | 类型 | Git 策略 | 作用 |
|---|---|---|---|
| `src/` | 项目提交代码 | 跟踪 | HyperSCA 核心包：数据处理、双曲模型、因果推断、扰动分析、靶点发现、行为语法 sidecar、评估和可视化。 |
| `scripts/` | 项目入口脚本 | 跟踪 | 数据入库、示例、Step1-Step4、靶点发现、行为语法模拟、图表生成、环境验证等可运行入口。 |
| `tests/` | 验证代码 | 跟踪 | 几何、因果、扰动、靶点发现、行为语法、MSI 推断、环境验证等测试。 |
...
| `C:\h` | 本地 Python 环境 | 仓库外 | 当前 HyperSCA 主线验证环境，Python 3.10.20；详见 `docs/research/hypersca_environment_alignment_2026-06-12.md`。 |
| `.venv/` | 本地 Python 环境 | 忽略 | 历史 Python 3.13 实验环境路径；本轮已清理，不作为当前运行入口。 |
...
| `src/behavior_grammar/` | Stage5 可选 sidecar：规则模型、受控 signal/behavior 字典、target discovery 证据到可读规则的转换、轻量虚拟组织模拟和 QoI sensitivity。 |
...
| 行为语法 sidecar | `scripts/run_behavior_grammar_simulation.py` | 从 target discovery manifest 与可选 Step4 context 导出规则、运行虚拟组织模拟，并写入 run-scoped artifact manifest。 |
...
| 行为语法与虚拟组织 | `tests/behavior_grammar/test_rules.py`, `tests/behavior_grammar/test_pipeline.py`, `tests/behavior_grammar/test_biological_scenarios.py` |
| 环境验证行为 | `tests/test_validate_env.py` |
| MSI 推断 | `tests/test_msi_inference.py` |

当前 `C:\h\python.exe` 环境下，`scripts/validate_env.py` 通过；本轮轻量回归 `tests/test_lorentz.py tests/test_causal_metrics.py` 为 `23 passed`。历史 `.venv` 全量测试记录为 `150 passed, 137 warnings`，作为旧快照参考；`scgen` 仍为 optional warning。
...
| `results/behavior_grammar/<run_id>/` | Stage5 sidecar 的默认输出路径。新运行会写入 `rules/rules.json`、`rules/rules.md`、`simulation/population_trajectory.csv`、`simulation/simulation_summary.json`、`simulation/qoi_sensitivity.csv`、`figures/population_trajectories.png` 和 run manifest。 |
...
| Stage5 行为语法 | 新增 `src/behavior_grammar/` 和 `scripts/run_behavior_grammar_simulation.py`，把 target discovery/Step4 证据转为可读规则与轻量虚拟组织轨迹；它是 sidecar，不替换 Step1-Step4。 |
...
| 2026-06-12 | v0.5 behavior grammar sidecar | 参考 Johnson et al. 2025 Cell 的 human-interpretable grammar 思路，新增可选 Stage5：规则模型、target discovery 证据转换、轻量虚拟组织模拟、QoI sensitivity 和 CRC TME 生物场景测试。 |

## 9. 当前项目进度和效果

- 主线运行环境仍兼容 Python 3.10；当前本机验证入口为 `C:\h\python.exe`，仓库内 `.venv` 已清理。
- Step1-Step4 已有可执行脚本、源码模块、本地 demo 输出和测试；Stage5 behavior grammar 作为可选 sidecar 接在 target discovery 或 Step4 之后。
...
- 当前 `C:\h\python.exe` 环境下环境验证通过，轻量回归通过；`scgen` 缺失仅作为 optional warning。
