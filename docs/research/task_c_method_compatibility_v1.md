...
候选参数文件的内容和 SHA-256 会同时写入运行身份、环境记录和方法状态，不能在方法结束后补写。正式参数选择只接收这种由统一命令封存的结果；不同细胞背景、不同跨环境方向或不同调节输入的结果不能混用。
...
单环境文件含表达、干预标签和基因名三个数组。跨环境文件另含环境标签；它恰好来自同一方向的 `source_refit.npz` 与 `target_adapt_refit.npz`。完成细胞和基因选择后，每个环境分别用至少两个未定向干预对照细胞计算均值和总体标准差，并按 `(表达值-对照均值)/对照标准差` 换算；标准差不大于 `1e-6` 时按 1 处理。变换名称固定为 `per_environment_control_zscore_then_row_concatenate_v1`，旧的“只减均值”名称不再接受。

记录文件保存所用规模、上限、种子、选择规则、公开父文件及其 SHA-256、基因名和原列号、每层干预的原行号、环境次序、输出数组摘要和输出文件 SHA-256。`refit` 与 `tune` 两个阶段使用同一个由公开 refit 父文件确定的基因清单；调节响应只投影这份既定清单，不参与挑选基因。单环境调节文件来自对应背景的 `tune.npz`；跨环境调节文件来自同一方向的 `source_tune.npz` 和 `target_adapt_tune.npz`。核验程序会从公开父文件重新执行选择与换算，并逐数组比较。父文件、阶段、方向、行号、环境标签、基因顺序或输出内容任一项变化都会停止运行。该接口只接受登记的公开学习数据，不接受合成数据，也不读取独立验证数据。
