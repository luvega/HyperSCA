...
基因确定后，只保留未定向干预对照和这些已选基因的干预细胞；`excluded`、环境特有干预和未选基因干预全部在抽样前删除，避免它们占用有限的细胞名额或为观察信息方法增加额外信息。记录文件保存被删除的原行号、数量和按标签汇总。剩余细胞再按每种标签使用固定种子分层抽取，并记录排序后的原行号和抽取前后数量。因此，不同方法看到的是同一批基因和同一批细胞，而不是仅仅拥有相同的数量上限。
...
核验完成后，统一命令把已核验的比较范围字节写入自己控制的临时文件，再交给外部工作进程或 HyperSCA-C；工作进程不会重新打开可能已被替换的原路径。运行结束前，公开清单、父文件和原比较范围记录还会再次核对。HyperSCA-C 的运行身份使用原比较范围的稳定记录路径和内容指纹，实际读取路径可以是临时副本，因此安全复制不会破坏后续复用身份。
