...
记录文件保存所用规模、上限、种子、选择规则、公开父文件及其 SHA-256、基因名和原列号、每层干预的原行号、环境次序、输出数组摘要和输出文件 SHA-256。核验程序会从公开父文件重新执行选择与换算，并逐数组比较。父文件、方向、行号、环境标签、基因顺序或输出内容任一项变化都会停止运行。该接口只接受登记的公开学习数据，不接受合成数据，也不读取独立验证数据。
